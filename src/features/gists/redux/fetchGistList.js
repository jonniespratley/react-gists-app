...
export function fetchGistList(args = {}) {
  return (dispatch) => { // optionally you can have getState as the second argument
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      const doRequest = args.error ? Promise.reject(new Error()) : getGists();
      doRequest.then(
        (res) => {
          dispatch({
            type: GISTS_FETCH_GIST_LIST_SUCCESS,
            data: res,
          });
          resolve(res);
        },
...
export function dismissFetchGistListError() {
  return {
...
export function reducer(state, action) {
  switch (action.type) {
    case GISTS_FETCH_GIST_LIST_BEGIN:
      ...
      return {
        ...state,
        fetchGistListPending: true,
        fetchGistListError: null,
      };

    case GISTS_FETCH_GIST_LIST_SUCCESS:
    ...
    case GISTS_FETCH_GIST_LIST_FAILURE:
      // The request is failed
      return {
        ...state,
        fetchGistListPending: false,
        fetchGistListError: action.data.error,
      };

    case GISTS_FETCH_GIST_LIST_DISMISS_ERROR:
